...
# st.image("path_to_logo.jpg", widFth=100)  # Replace with the path to your logo
# Create two columns
...
# Sample SVG for demonstration purposes
svg_icon_Dashboard = """
...
st.sidebar.markdown(f'<div class="hoverable" style="cursor: pointer; padding:10px 10px; width: 150px; border-radius: 4px; transition:all ease-in-out 300ms; display:flex; gap:10px; align-items:center">{svg_icon_Dashboard} Dashboard</div>', unsafe_allow_html=True)
st.sidebar.markdown(f'<div class="hoverable" style="cursor: pointer; padding:10px 10px; width: 150px; border-radius: 4px; transition:all ease-in-out 300ms; display:flex; gap:10px; align-items:center">{svg_icon_Projects} Projects</div>', unsafe_allow_html=True)
st.sidebar.markdown(f'<div class="hoverable" style="cursor: pointer; padding:10px 10px; width: 150px; border-radius: 4px; transition:all ease-in-out 300ms; display:flex; gap:10px; align-items:center">{svg_icon} User</div>', unsafe_allow_html=True)
st.sidebar.markdown(f'<div class="hoverable" style="cursor: pointer; padding:10px 10px; width: 150px; border-radius: 4px; transition:all ease-in-out 300ms; display:flex; gap:10px; align-items:center">{svg_icon_ProjectStatus} Projects Status</div>', unsafe_allow_html=True)
...
with col1:
    # Custom HTML & CSS to mimic st.metric but with value and label swapped
    # metric_html = """
    # <div style="k">
    #     <div style="font-size: 1.5rem; font-weight: bold;">654</div>
    #     <div style="font-size: 1rem;">Projects</div>
    #     <div style="font-size: 0.85rem; color: #14DE3F; margin-top: 0.25rem;">+3%</div>
    # </div>
    # """
...
# Additional sections can be added below as per the requirement.

# response = requests.get(
#     'https://tuk-cdn.s3.amazonaws.com/can-uploader/MicrosoftTeams-image551ad57e01403f080a9df51975ac40b6efba82553c323a742b42b1c71c1e45f1.jpg')
...
# # Display label and delta
# st.text(label)
# st.text(f"Delta: {delta}")
